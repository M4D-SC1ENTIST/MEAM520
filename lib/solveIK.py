...
class IK:
    ...
    def displacement_and_axis(target, current):
        ...
        x_displacement = target[0, 3] - current[0, 3]
        y_displacement = target[1, 3] - current[1, 3]
        z_displacement = target[2, 3] - current[2, 3]
        displacement = np.array([x_displacement, y_displacement, z_displacement])

        R_current = current[0:3, 0:3]
        R_target = target[0:3, 0:3]

        R_current_inverse = inv(R_current)
        R_current_target = R_current_inverse @ R_target

        S = (1/2) * (R_current_target - np.transpose(R_current_target))
        a1 = S[2, 1]
        a2 = S[0, 2]
        a3 = S[1, 0]
    ...
    def distance_and_angle(G, H):
        ...
        G_displacement = np.array([G[0, 3], G[1, 3], G[2, 3]])
        H_displacement = np.array([H[0, 3], H[1, 3], H[2, 3]])
        distance = np.linalg.norm(H_displacement - G_displacement)
        ...
        R_G_inverse = inv(R_G)
        R_GH = R_G_inverse @ R_H

        #R_GH = np.cross(R_G, R_H)

        input_to_arccos = (np.trace(R_GH) - 1)/2

        if input_to_arccos > 1:
            input_to_arccos = 1
        elif input_to_arccos < -1:
            input_to_arccos = -1
        
        angle = np.arccos(input_to_arccos)
